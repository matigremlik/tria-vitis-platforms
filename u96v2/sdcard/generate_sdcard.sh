...
copy_optional_app_tar() {
    ...
    if [ -n "${APP_TAR_PATH:-}" ]; then
        app_tar="${APP_TAR_PATH}"
    elif [ -e "./app.tar.gz" ]; then
        app_tar="./app.tar.gz"
    elif [ -e "/mnt/m/app.tar.gz" ]; then
        app_tar="/mnt/m/app.tar.gz"
    fi

    if [ -z "${app_tar}" ]; then
        msg "Copying app bundle         "
        ret=0
        report
        return
    fi
    ...
    msg "Copying app bundle         "
    mkdir -p binary.tmp/home/root
    cp -f "${app_tar}" binary.tmp/home/root/app.tar.gz
    sync
    ret="$?"
    report
}
